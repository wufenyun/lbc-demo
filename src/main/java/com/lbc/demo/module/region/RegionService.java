...
public class RegionService extends ServiceBase {
    ...
    public Result listAll(String key) {
        QueryingCollection<RegionDto> queryingCollection = localCache.get(key);
        return success(queryingCollection.generateTrees());
    }

    public Result listByKey(String key) {
        QueryingCollection<RegionDto> queryingCollection = localCache.get(key, RegionLoader.class);
        return success(queryingCollection.generateTrees());
    }
}